...
const AnalyticsAllWeek = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/analytics/questions?q=week')
        .then(response => {
        ...
        axios.get('http://localhost:8000/analytics/all?q=week')
            .then(response => {
    ...
    {questionsData.slice(0, -1).map(question => (
        ...
        <div className='analyticsAll'>
            <h2>Все оценки за последнюю неделю</h2>
            <ul>
...
export default AnalyticsAllWeek;
