...
const AnalyticsAllMonth = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/analytics/questions?q=month')
        .then(response => {
        ...
        axios.get('http://localhost:8000/analytics/all?q=month')
            .then(response => {
    ...
    {questionsData.slice(0, -1).map(question => (
        <li key={question.id}>
            ...
            <h2>Все оценки за последний месяц</h2>
            <ul>
...
export default AnalyticsAllMonth;
