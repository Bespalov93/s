...
const AnalyticsAllDay = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/analytics/questions?q=day')
        .then(response => {
        ...
        axios.get('http://localhost:8000/analytics/all?q=day')
            .then(response => {
    ...
    {questionsData.slice(0, -1).map(question => (
        ...
        <div className='analyticsAll'>
            <h2>Все оценки за последний день</h2>
            <ul>
...
export default AnalyticsAllDay;
